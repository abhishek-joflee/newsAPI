...
def update_top_headline():
    for category in CATEGORIES:
        for country in COUNTRIES_LANGUAGES:
            print(
                "Started category:{0} country:{1} at :{2}".format(
                    category, country, time.strftime("%A, %d. %B %Y %I:%M:%S %p")
                )
            )
            ...
            push_to_github(
                "top-headlines/category/{0}/{1}.json".format(category, country),
                top_headlines,
            )
...
def update_everything():
    ...
    for source in SOURCES:
        print(
            "Started source:{0} : {1}".format(
                source, time.strftime("%A, %d. %B %Y %I:%M:%S %p")
            )
        )
        all_articles = newsapi.get_everything(
            sources=source,
            from_param=(datetime.now() - timedelta(days=1, hours=5, minutes=30))
            .date()
            .isoformat(),
            language="en",
            sort_by="publishedAt",
